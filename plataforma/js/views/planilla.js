...
export async function render(host) {
  ...
  host.append(
    card(
      'Periodos',
      session.is('admin') ? h('button', { class: 'btn btn--ghost btn--sm', text: 'Abrir periodo', onClick: newPeriod }) : null,
      periodList,
      true
    ),
    detail
      ...
      h(
        ...
        card(data.period.name, actions, [
          h('div', {
            class: 'hint',
            text: open
              ? 'Este periodo esta abierto: los consumos siguen sumando. Cierralo cuando entregues el reporte a Recursos Humanos.'
  ...
  function closePeriod(data) {
    confirmAction({
      ...
      danger: true,
      onConfirm: async () => {
        await api.closePeriod(data.period.id);
        toast('Periodo cerrado y descuentos aplicados');
        selected = null;
        await loadPeriods();
      }
    });
  ...
  function newPeriod() {
    const name = input({ placeholder: '1a quincena agosto 2026' });
    const start = input({ type: 'date' });
    const end = input({ type: 'date' });
    const dialog = modal({
      title: 'Abrir periodo',
      body: [
        h('div', { class: 'hint', text: 'Debes cerrar el periodo abierto antes de abrir otro.' }),
