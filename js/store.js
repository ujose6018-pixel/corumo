...
/* ================= ciclos de planilla ================= */
// El corte puede ser semanal, quincenal o mensual. Todo se calcula en fechas
// simples (YYYY-MM-DD) para que no dependa de la zona horaria del navegador.
...
/** Rango del periodo que contiene la fecha dada, segun el ciclo. */
export function periodRange(cycle, ref = new Date()) {
  const base = ref instanceof Date ? ref : parseDate(ref);
  ...
  if (cycle === 'semanal') {
    ...
    const mismoMes = start.getUTCMonth() === end.getUTCMonth();
    const nombre = mismoMes
      ? `Semana del ${start.getUTCDate()} al ${end.getUTCDate()} de ${MESES[start.getUTCMonth()]} ${end.getUTCFullYear()}`
      : `Semana del ${start.getUTCDate()} de ${MESES[start.getUTCMonth()]} al ${end.getUTCDate()} de ${MESES[end.getUTCMonth()]} ${end.getUTCFullYear()}`;
    return { nombre, inicio, fin };
  }

  if (cycle === 'mensual') {
    return {
      nombre: `Mes de ${MESES[m]} ${y}`,
      inicio: ymd(y, m, 1),
      fin: ymd(y, m, lastDay(y, m))
    };
  }

  // Quincenal: 1 al 15 y 16 al fin de mes.
  const primera = d <= 15;
  return {
    nombre: `${primera ? '1a' : '2a'} quincena ${MESES[m]} ${y}`,
    inicio: ymd(y, m, primera ? 1 : 16),
    fin: primera ? ymd(y, m, 15) : ymd(y, m, lastDay(y, m))
  };
}

/** Rango que sigue justo despues de una fecha de cierre. */
export function nextPeriodRange(cycle, afterISO) {
  const next = new Date(parseDate(afterISO).getTime() + 86400000);
  return periodRange(cycle, next);
}
...
export const api = {
  ...
  /** Propone el siguiente corte: continua donde termino el ultimo, o el que contiene hoy. */
  suggestPeriod: guard(async () => {
    const settings = await api.settings();
    const cycle = settings.payroll_cycle;
    const all = await api.periods();
    const last = all.find((p) => p.end_date);
    const range = last ? nextPeriodRange(cycle, last.end_date) : periodRange(cycle);
    return { cycle, cycleLabel: CYCLES[cycle], ...range };
  }),
  ...
  closePeriod: guard(async (id) => {
    ...
    // El consumo no se detiene: se abre de una vez el corte siguiente.
    let next = null;
    const settings = await api.settings();
    if (settings.auto_open_period !== '0') {
      try {
        const range = nextPeriodRange(settings.payroll_cycle, data.period.end_date);
        const ref = await addDoc(C.periodos, {
          nombre: range.nombre,
          inicio: range.inicio,
          fin: range.fin,
        ...
        next = { id: ref.id, name: range.nombre };
        log('crear_periodo', 'periodos', ref.id, `${range.nombre} (automatico)`);
      } catch {
        // Si no se pudo abrir, el administrador lo abre a mano desde Planilla.
      }
    }

    return { ok: true, aplicados: pending.length, next };
  }),
  ...
  settings: guard(async () => {
    ...
    return {
      ...
      payroll_cycle: CYCLES[d.cicloPlanilla] ? d.cicloPlanilla : 'quincenal',
      auto_open_period: d.autoAbrirPeriodo === false ? '0' : '1'
    };
  }),
  ...
  saveSettings: guard(async (data) => {
    await setDoc(
      doc(dbf, 'config', 'general'),
      {
        ...
        cicloPlanilla: CYCLES[data.payroll_cycle] ? data.payroll_cycle : 'quincenal',
        autoAbrirPeriodo: data.auto_open_period !== '0'
      },
      { merge: true }
    );
